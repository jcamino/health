<script lang="ts">
  ...
  const textColor: Record<string, string> = {
    ...
    "stage-1": "text-orange-600 dark:text-orange-400",
    "stage-2": "text-red-600 dark:text-red-400",
  ...
  const zonesPlugin = {
    ...
    beforeDatasetsDraw(c: Chart) {
      ...
      const px = (v: number) =>
        Math.max(
          chartArea.left,
          Math.min(chartArea.right, scales.x.getPixelForValue(v)),
        );
      const py = (v: number) =>
        Math.max(
          chartArea.top,
          Math.min(chartArea.bottom, scales.y.getPixelForValue(v)),
        );
      const fill = (
        d0: number,
        d1: number,
  ...
  function render() {
    ...
    if (result) {
      ...
      (ds as { pointBackgroundColor?: string }).pointBackgroundColor =
        pointColor[result.category];
    } else {
      ds.data = [];
    }
    chart.update();
      ...
      options: {
        ...
        plugins: {
          ...
          tooltip: {
            callbacks: {
              label: () =>
                `${systolic}/${diastolic} mmHg${result ? `: ${result.label}` : ""}`,
            },
          },
        },
      },
...
<div class="card not-prose">
  ...
  {#if result}
    <p class="mt-4 text-sm">
      Category: <span class={`font-semibold ${textColor[result.category]}`}
        >{result.label}</span
      >
    </p>
  {:else}
  ...
  <div class="mt-4 h-80">
    <canvas
      bind:this={canvas}
      aria-label="Blood-pressure category chart: diastolic on the x-axis, systolic on the y-axis, with colored ACC/AHA zones and your reading plotted as a point"
    ></canvas>
  </div>
  <p class="hint">
    Zones follow the 2017 ACC/AHA categories; because the higher of the systolic
    and diastolic categories applies, the bands are L-shaped. Your reading is
    the dot. The dashed green box is the prevention-optimal target: the 2025
    ACC/AHA guidance to get
    <span class="whitespace-nowrap">as close to under 120/80 as tolerated</span
    >. Judge yourself against home readings (rested, seated, averaged); office
    readings run high.
  </p>
