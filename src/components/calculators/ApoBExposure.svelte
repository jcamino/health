<script lang="ts">
  ...
  const inputsValid = $derived(
    [currentAge, currentValue, risePerDecade].every((n) =>
      Number.isFinite(n),
    ) &&
      currentValue > 0 &&
      (!useIntervention ||
        ([interventionAge, interventionValue].every((n) =>
          Number.isFinite(n),
        ) &&
          interventionValue > 0)),
  );
  ...
  const totalExposure = $derived(
    scenario ? Math.round(apoBYears(scenario)) : null,
  );
  const crossingAge = $derived(
    scenario
      ? ageAtThreshold(scenario, CUMULATIVE_EXPOSURE_THRESHOLD_MG_YEARS)
      : null,
  );
  ...
  const displaySources = $derived(
    inputMode === "apoB"
      ? exposureSources
      : [...exposureSources, ...conversionSources],
  );
  ...
  const crosshairPlugin = {
    ...
    afterDraw(c: Chart) {
      ...
      ctx.setLineDash([4, 4]);
      ctx.lineWidth = 1;
      ctx.strokeStyle = "rgba(138, 134, 126, 0.8)"; // mid ink-muted, legible on both themes
  ...
  function readHover(c: Chart) {
    ...
    const pt = c.data.datasets[0].data[active[0].index] as
      | { x: number; y: number }
      | undefined;
        ...
        datasets: [
          {
            ...
            pointHoverRadius: 5,
            pointHitRadius: 20,
            // Mid-tone oxblood: reads on both the light and dark theme (charts
            // don't re-render on theme toggle).
            borderColor: "#a03a4c",
            backgroundColor: "rgba(160, 58, 76, 0.14)",
          },
          {
      ...
      options: {
        ...
        plugins: {
          tooltip: {
            callbacks: {
              title: (items) =>
                items.length ? `Age ${items[0].parsed.x}` : "",
              label: (item) => {
...
<div class="card not-prose">
  <div class="grid grid-cols-2 items-end gap-3 sm:grid-cols-4">
    <label class="field"
      >Enter as
      <select bind:value={inputMode} class="input w-full">
    ...
    >
    <label class="field"
      >ApoB rise per decade (mg/dL)
      <input
        type="number"
        min="0"
        max="50"
        bind:value={risePerDecade}
        class="input w-full"
      /></label
    >
  </div>
  ...
  {#if estimatedApoB !== null}
    <p class="mt-2 text-xs text-ink-muted">
      Estimated ApoB ≈ <strong class="font-mono">{estimatedApoB} mg/dL</strong>
      (converted from {unitLabel}).
    </p>
  {/if}

  <label class="mt-4 flex items-center gap-2 text-sm">
    <input type="checkbox" bind:checked={useIntervention} /> Model an intervention
    (e.g. a statin lowers it, then held)
  </label>
  {#if useIntervention}
    <div class="mt-2 grid grid-cols-2 items-end gap-3">
      <label class="field"
        ...
        <input
          ...
          min="1"
          max="99"
          bind:value={interventionAge}
          class="input w-full"
        /></label
      >
      <label class="field"
        >{unitLabel} after (mg/dL)
        <input
          type="number"
          min="1"
          max="400"
          bind:value={interventionValue}
          class="input w-full"
        /></label
      >
    </div>
  ...
  <div class="mt-4 h-64">
    <canvas
      bind:this={canvas}
      aria-label="Cumulative ApoB exposure from birth over age"
    ></canvas>
  </div>
  ...
  {#if hovered}
    <p class="mt-2 text-sm">
      ...
      <strong class="font-mono"
        >{Math.round(hovered.cumulative).toLocaleString()} mg·years</strong
      >
      cumulative ({Math.round(
        (100 * hovered.cumulative) / CUMULATIVE_EXPOSURE_THRESHOLD_MG_YEARS,
      )}% of the illustrative threshold).
    </p>
  {:else}
    <p class="mt-2 text-xs text-ink-muted">
  ...
  {#if scenario}
    <p class="mt-3 text-sm">
      Cumulative exposure from birth to age {endAge}:
      <strong class="font-mono">{totalExposure} mg·years</strong>.
      {#if crossingAge !== null}
  ...
  {:else}
    <p class="mt-3 text-sm text-red-600 dark:text-red-400">
      Check the inputs: all fields must be positive numbers, and any
      intervention age must be under {endAge}.
    </p>
  {/if}

  <p class="hint">
    ApoB or non-HDL-C are preferred; LDL-C is converted approximately and
