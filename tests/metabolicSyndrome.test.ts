...
describe("metabolicSyndrome (harmonized 2009, 3 of 5)", () => {
  it("counts the criteria met", () => {
    ...
    expect(metabolicSyndrome({ ...none, waist: true, hdl: true }).count).toBe(
      2,
    );
  });

  it("requires at least 3 of 5 for a diagnosis (boundary)", () => {
    expect(metabolicSyndrome({ ...none, waist: true, hdl: true }).meets).toBe(
      false,
    ); // 2
  ...
  it("ships sources including the harmonized statement", () => {
    ...
    expect(
      sources.some((s) => s.doi === "10.1161/CIRCULATIONAHA.109.192644"),
    ).toBe(true);
  });
});
...
describe("evaluateCriteria (thresholds from the harmonized table)", () => {
  ...
  it("honors HDL sex-specific cut-points (<40 men, <50 women)", () => {
    ...
    expect(evaluateCriteria({ ...base, sex: "male", hdlMgDl: 45 }).hdl).toBe(
      false,
    ); // 45 not <40
    expect(evaluateCriteria({ ...base, sex: "female", hdlMgDl: 45 }).hdl).toBe(
      true,
    ); // 45 <50
  });
